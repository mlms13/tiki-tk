...
App.prototype = $extend(doom_html_Component.prototype,{
	...
	,demoSection: function(title,children) {
		...
		return doom_core__$VNode_VNode_$Impl_$.el("section",attributes,doom_core__$VNodes_VNodes_$Impl_$.children([tmp,doom_core__$VNode_VNode_$Impl_$.el("div",_g2,children)]));
	}
...
doom_html_Render.prototype = {
	...
	,renderComponent: function(comp) {
		try {
			...
			throw new thx_error_ErrorWrapper("unable to render " + typeName,e,null,{ fileName : "Render.hx", lineNumber : 228, className : "doom.html.Render", methodName : "renderComponent"});
		}
	}
	...
	,applyNodeAttributes: function(attributes,dom) {
		...
		while(tmp3.hasNext()) {
			...
			case 1:
				var s = _g23[2];
				if(null == s && key3 == "value") doom_html_Attributes.setStringAttribute(dom,key3,""); else {
					var s3 = _g23[2];
					if(null == s3 || s3 == "" && key3 != "value") doom_html_Attributes.removeAttribute(dom,key3); else {
						var s4 = _g23[2];
						doom_html_Attributes.setStringAttribute(dom,key3,s4);
					}
				}
				break;
			case 0:
	...
	,createElement: function(name,attributes,children,post) {
		...
		if(colonPos > 0) {
			...
			var ns = tmp;
			if(null == ns) throw new thx_Error("element prefix \"" + prefix + "\" is not associated to any namespace. Add the right namespace to Doom.namespaces.",null,{ fileName : "Render.hx", lineNumber : 369, className : "doom.html.Render", methodName : "createElement"});
...
tiki_Button.getClasses = function(base,state) {
	...
	var styleClass;
	var _g = state.type;
	if(_g == null) styleClass = ""; else switch(_g[1]) {
	case 1:
		styleClass = "primary";
		break;
	case 2:
	...
	if(state.hollow == true) styleClass += "-hollow";
	classes.push(styleClass);
	if(state.active == true) classes.push("active");
	if(state.disabled == true) classes.push("disabled");
	var tmp;
	var _g1 = state.size;
	if(_g1 == null) tmp = []; else switch(_g1[1]) {
	case 0:
		tmp = ["large"];
		break;
	case 1:
...
tiki_ButtonSize.Default.toString = $estr;
...
tiki_Nav.getClasses = function(state) {
	...
	var tmp;
	var _g = state.layout;
	if(_g == null) tmp = ""; else switch(_g[1]) {
	case 0:
		tmp = "nav-inline";
		break;
	case 1:
	...
	classes.push(tmp);
	if(state.stacked == true) classes.push("nav-stacked");
	return classes.join(" ");
};
