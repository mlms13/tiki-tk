...
doom_html_Render.prototype = {
	...
	,applyNodeAttributes: function(attributes,dom) {
		...
		while(tmp3.hasNext()) {
			...
			if(_g21 == null) {
				doom_html_Attributes.removeAttribute(dom,key2);
			} else {
				switch(_g21[1]) {
				case 1:
					var s = _g21[2];
	...
	,createElement: function(name,attributes,children,post) {
		...
		if(colonPos > 0) {
			...
			var ns = tmp;
			if(null == ns) {
				throw new thx_Error("element prefix \"" + prefix + "\" is not associated to any namespace. Add the right namespace to Doom.namespaces.",null,{ fileName : "Render.hx", lineNumber : 367, className : "doom.html.Render", methodName : "createElement"});
			}
...
dots_Dom.scrollTop = function(doc) {
	...
	if(null != window.document.documentElement) {
...
thx_Arrays.containsAll = function(array,elements,eq) {
	...
	while(tmp.hasNext()) {
		var el = tmp.next();
		if(!thx_Arrays.contains(array,el,eq)) {
			return false;
		}
	}
	return true;
...
thx_Arrays.spanByIndex = function(arr,spanKey) {
	...
	while(_g1 < _g) {
		...
		var k = spanKey(i);
		if(k == null) {
			throw new thx_Error("spanKey function returned null for index " + i,null,{ fileName : "Arrays.hx", lineNumber : 497, className : "thx.Arrays", methodName : "spanByIndex"});
		}
		if(cur == k) {
			acc[j].push(arr[i]);
		} else {
...
tiki_Button.getClasses = function(state) {
	var classes = ["btn"];
	var styleClass;
	var _g = state.type;
	if(_g == null) {
...
tiki_Button.prototype = $extend(doom_html_Component.prototype,{
	render: function() {
		...
		var value1 = doom_core__$AttributeValue_AttributeValue_$Impl_$.fromString(tiki_Button.getClasses(this.props));
		if(__map_reserved["class"] != null) {
