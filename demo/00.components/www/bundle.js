...
var App = function(api,state,children) {
	this.api = api;
	this.state = state;
...
App.__name__ = ["App"];
App["with"] = function(children) {
	var apiVar = { };
	var stateVar = { };
	return doom_NodeImpl.ComponentNode(new App(apiVar,stateVar,children));
};
App.__super__ = Doom;
App.prototype = $extend(Doom.prototype,{
	...
	,demoSection: function(title,children) {
		var _g = new haxe_ds_StringMap();
		var value = doom__$AttributeValue_AttributeValue_$Impl_$.fromString("demo-section");
		if(__map_reserved["class"] != null) _g.setReserved("class",value); else _g.h["class"] = value;
		var attributes = _g;
		var _g1 = new haxe_ds_StringMap();
		var value1 = doom__$AttributeValue_AttributeValue_$Impl_$.fromString("section-title");
		if(__map_reserved["class"] != null) _g1.setReserved("class",value1); else _g1.h["class"] = value1;
		var tmp = doom__$Node_Node_$Impl_$.el("h2",_g1,null,doom_NodeImpl.Text(title));
...
Main.__name__ = ["Main"];
Main.main = function() {
	Doom.mount(App["with"](),window.document.querySelector("#root"));
};
Math.__name__ = ["Math"];
...
var tiki_Button = function(api,state,children) {
	if(state.type == null) state.type = tiki_ButtonType.Default;
	this.api = api;
	this.state = state;
	this.children = children;
	Doom.call(this,children);
...
tiki_Button["with"] = function(state,children) {
	var apiVar = { };
	if(state == null) state = { };
	var stateVar = { type : state.type};
	return doom_NodeImpl.ComponentNode(new tiki_Button(apiVar,stateVar,children));
};
tiki_Button.__super__ = Doom;
tiki_Button.prototype = $extend(Doom.prototype,{
	render: function() {
		var _g = new haxe_ds_StringMap();
		...
		var value1 = doom__$AttributeValue_AttributeValue_$Impl_$.fromString(this.getClasses());
...
doom_HtmlNode.applyPatch = function(patch,node) {
	...
	case 9:
		...
		case 1:
			var name3 = patch[2];
			var value = patch[3];
			if(value == null) node.removeAttribute(name3); else switch(value[1]) {
			case 1:
				var s = value[2];
				if(s == null || s == "") node.removeAttribute(name3); else {
					var s3 = value[2];
					node.setAttribute(name3,s3);
				}
				break;
			case 0:
				var b = value[2];
				if(b) node.setAttribute(name3,name3); else node.removeAttribute(name3);
				break;
			case 2:
				var e = value[2];
				doom_HtmlNode.addEvent(node,name3,e);
				break;
			}
			break;
		default:
		...
		break;
	case 10:
		var children3 = patch[4];
		var attributes3 = patch[3];
		var name4 = patch[2];
		var parent = node.parentNode;
		var post4 = [];
		var el4 = doom_HtmlNode.createElement(name4,attributes3,children3,post4);
		parent.replaceChild(el4,node);
		var _g14 = 0;
		while(_g14 < post4.length) {
			var f4 = post4[_g14];
			++_g14;
			f4();
		}
	...
	case 13:
		...
		var post5 = [];
		comp4.init(post5);
		parent4.replaceChild(comp4.element,node);
		var _g15 = 0;
		while(_g15 < post5.length) {
			var f5 = post5[_g15];
			++_g15;
			f5();
		}
	...
	case 11:
		var text5 = patch[2];
		var parent5 = node.parentNode;
		parent5.replaceChild(window.document.createTextNode(text5),node);
		break;
	case 12:
		var raw = patch[2];
		var parent6 = node.parentNode;
		parent6.replaceChild(dots_Html.parse(raw),node);
		break;
	case 14:
		switch(_g) {
		case 3:
			...
			break;
		case 8:
			var newcontent6 = patch[2];
			if(node.parentNode.nodeName == "TEXTAREA") node.parentNode.value = newcontent6; else node.nodeValue = newcontent6;
			break;
		default:
			throw new thx_Error("cannot apply patch " + Std.string(p) + " on " + Std.string(node),null,{ fileName : "HtmlNode.hx", lineNumber : 154, className : "doom.HtmlNode", methodName : "applyPatch"});
		}
...
doom__$Node_Node_$Impl_$.diffAttributes = function(a,b) {
	...
	var tmp4 = $iterator(thx__$Set_Set_$Impl_$)(kb);
	while(tmp4.hasNext()) {
		var item = tmp4.next();
		result.remove(item);
	}
	...
	var tmp5 = $iterator(thx__$Set_Set_$Impl_$)(ka);
	while(tmp5.hasNext()) {
		var item5 = tmp5.next();
		result1.remove(item5);
	}
	var added = result1;
	var result2 = thx__$Set_Set_$Impl_$.empty(ka);
	var tmp6 = $iterator(thx__$Set_Set_$Impl_$)(ka);
	while(tmp6.hasNext()) {
		var item6 = tmp6.next();
		var tmp7;
		var _this = kb;
		if(__map_reserved[item6] != null) tmp7 = _this.existsReserved(item6); else tmp7 = _this.h.hasOwnProperty(item6);
		if(tmp7) {
			var _this7 = result2;
			if(__map_reserved[item6] != null) _this7.setReserved(item6,true); else _this7.h[item6] = true;
		}
	}
	var common = result2;
	var _e = removed;
	...
	var _e2 = common;
	var tmp2 = tmp.concat(thx__$Set_Set_$Impl_$.map((function(predicate) {
	...
	var _e3 = added;
	var _e4 = (function(predicate3) {
		return thx__$Set_Set_$Impl_$.filter(_e3,predicate3);
	})(function(_3) {
		return (__map_reserved[_3] != null?b.getReserved(_3):b.h[_3]) != null;
	});
	return tmp2.concat((function(f7) {
		return thx__$Set_Set_$Impl_$.map(_e4,f7);
	})(function(_7) {
		return doom_Patch.SetAttribute(_7,__map_reserved[_7] != null?b.getReserved(_7):b.h[_7]);
	}));
};
...
doom__$Node_Node_$Impl_$.diffNodes = function(a,b) {
	...
	while(_g11 < _g2) {
		var i2 = _g11++;
		result = result.concat(doom__$Node_Node_$Impl_$.diffAdd(b[i2]));
	}
	var _g12 = 0;
	var _g3 = min;
	while(_g12 < _g3) {
		var i3 = _g12++;
		var diff = doom__$Node_Node_$Impl_$.diff(a[i3],b[i3]);
		if(diff.length > 0) result.push(doom_Patch.PatchChild(i3,diff));
	}
	return result;
};
doom__$Node_Node_$Impl_$.diff = function(this1,that) {
	...
	destroySubComponents = function(node) {
		...
		case 3:
			var comp = node[2];
			return [doom_Patch.DestroyComponent(comp)].concat(destroySubComponents(comp.node));
		default:
			return [];
		}
	};
	...
	case 3:
		...
		case 3:
			var old3 = this1[2];
			var comp4 = that[2];
			tmp = doom__$Node_Node_$Impl_$.diff(old3.node,comp4.node);
			break;
		case 0:
			var c2 = that[4];
			var a2 = that[3];
	...
	case 0:
		...
		case 0:
			...
			var a25 = that[3];
			var n25 = that[2];
			if(n1 != n25) tmp = [doom_Patch.ReplaceWithElement(n25,a25,c25)]; else {
				var c15 = this1[4];
				var a15 = this1[3];
				var c26 = that[4];
				var a26 = that[3];
				tmp = doom__$Node_Node_$Impl_$.diffAttributes(a15,a26).concat(doom__$Node_Node_$Impl_$.diffNodes(c15,c26));
			}
			break;
		case 2:
			var t6 = that[2];
	...
	case 2:
		...
		case 0:
			...
			var a27 = that[3];
			var n27 = that[2];
			tmp = [doom_Patch.ReplaceWithElement(n27,a27,c27)];
			break;
		case 2:
			var t1 = this1[2];
			var t2 = that[2];
			if(t1 != t2) tmp = [doom_Patch.ContentChanged(t2)]; else {
				var a = this1[2];
				var b = that[2];
				if(a == b) tmp = []; else {
					var t8 = that[2];
	...
	case 1:
		...
		case 0:
			...
			var a29 = that[3];
			var n29 = that[2];
			tmp = [doom_Patch.ReplaceWithElement(n29,a29,c29)];
			break;
		case 2:
			var t10 = that[2];
		...
		case 1:
			...
			if(a10 == b10) tmp = []; else {
				var t11 = that[2];
				tmp = [doom_Patch.ReplaceWithRaw(t11)];
			}
			break;
		}
		break;
	default:
		...
		case 3:
			var comp11 = that[2];
			tmp = [doom_Patch.ReplaceWithComponent(comp11)];
			break;
		case 0:
			var c211 = that[4];
			var a211 = that[3];
			var n211 = that[2];
			tmp = [doom_Patch.ReplaceWithElement(n211,a211,c211)];
			break;
		case 2:
			var t12 = that[2];
			tmp = [doom_Patch.ReplaceWithText(t12)];
			break;
		case 1:
			var t13 = that[2];
			tmp = [doom_Patch.ReplaceWithRaw(t13)];
			break;
		}
	}
	return p.concat(tmp);
...
doom_XmlNode.applyPatch = function(patch,node) {
	...
	case 9:
		...
		case 0:
			...
			case 1:
				var s = value[2];
				if(s != null && s.length > 0) node.set(name5,s); else node.remove(name5);
				break;
			case 0:
				var b = value[2];
				if(b) node.set(name5,name5); else node.remove(name5);
				break;
			default:
			}
			break;
		...
		break;
	case 10:
		var children5 = patch[4];
		var attributes5 = patch[3];
		var name6 = patch[2];
		...
		parent.insertChild(doom_XmlNode.createElement(name6,attributes5,children5),pos);
		break;
	case 13:
		var n6 = _g;
		switch(_g) {
	...
	case 11:
		var text6 = patch[2];
		var parent6 = node.parent;
		if(parent6.nodeType != Xml.Document && parent6.nodeType != Xml.Element) throw new js__$Boot_HaxeError("Bad node type, expected Element or Document but found " + parent6.nodeType);
		var pos6 = thx_Iterators.indexOf(HxOverrides.iter(parent6.children),node);
		parent6.removeChild(node);
		parent6.insertChild(Xml.createPCData(text6),pos6);
		break;
	case 12:
		var raw = patch[2];
		var parent7 = node.parent;
		if(parent7.nodeType != Xml.Document && parent7.nodeType != Xml.Element) throw new js__$Boot_HaxeError("Bad node type, expected Element or Document but found " + parent7.nodeType);
		var pos7 = thx_Iterators.indexOf(HxOverrides.iter(parent7.children),node);
		parent7.removeChild(node);
		parent7.insertChild(Xml.parse(raw),pos7);
		break;
	case 14:
		var n7 = _g;
		switch(_g) {
		case 2:
			...
			break;
		case 3:
			var newcontent7 = patch[2];
			if(node.nodeType == Xml.Document || node.nodeType == Xml.Element) throw new js__$Boot_HaxeError("Bad node type, unexpected " + node.nodeType);
			node.nodeValue = newcontent7;
			break;
		default:
	...
	while(c == c) {
		...
		case 14:
			if(c == 63 && str.charCodeAt(p + 1) == 62) {
				++p;
				var str5 = HxOverrides.substr(str,start + 1,p - start - 2);
				parent.addChild(Xml.createProcessingInstruction(str5));
				++nsubs;
				state = 1;
			}
			break;
		case 18:
			if(c == 59) {
				...
				if(s.charCodeAt(0) == 35) {
					var c5 = s.charCodeAt(1) == 120?Std.parseInt("0" + HxOverrides.substr(s,1,s.length - 1)):Std.parseInt(HxOverrides.substr(s,1,s.length - 1));
					buf.b += String.fromCharCode(c5);
				} else {
					var tmp5;
					var _this = haxe_xml_Parser.escapes;
					if(__map_reserved[s] != null) tmp5 = _this.existsReserved(s); else tmp5 = _this.h.hasOwnProperty(s);
					if(!tmp5) {
						if(strict) throw new js__$Boot_HaxeError(new haxe_xml_XmlParserException("Undefined entity: " + s,str,p));
						buf.b += Std.string("&" + s + ";");
					} else {
						var x;
						var _this5 = haxe_xml_Parser.escapes;
						if(__map_reserved[s] != null) x = _this5.getReserved(s); else x = _this5.h[s];
						buf.b += Std.string(x);
					}
				}
				start = p + 1;
